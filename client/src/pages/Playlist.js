...
const Playlist = () => {
	...
	useEffect(() => {
		...
		// By default, the api endpoint returns the first 100 tracks in the playlist
		...
		const allTracks = [...(tracks ? tracks : []), ...tracksData.items];

		// Remove issue with duplication of tracks that somtimes occures for very large playlists
		const filteredTracks = allTracks.filter((value, index, array) => array.findIndex((value2) => value2.track.id === value.track.id) === index);
						...
						<div className="header__inner">
							...
							<div>
								...
								<p className="header__meta">
									<span className="header__owner">{playlist.owner.display_name}</span>
									{playlist.followers.total ? (
										<span>
											{playlist.followers.total.toLocaleString()} {`follower${playlist.followers.total !== 1 ? "s" : ""}`}
										</span>
									) : null}
									<span>
